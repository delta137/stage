...
y1_initial = Bound_Cond(1);
y1_final = Bound_Cond(2);

y2_initial_min = Range_y2_initial(1);
y2_initial_max = Range_y2_initial(2);    
...
[x,y] = ode45(@(x,y)bvp(x,y,parametres),...
              [x_min x_max],...
              [y1_initial y2_initial_min+(y2_initial_max-y2_initial_min)/2],...
              options ) ;
...
if abs(err2)<conv
   err=err2;
   y2_initial=y2_initial_min+(y2_initial_max-y2_initial_min)/2;
   break
end
